...
```{r Exercise 1.9}
# write code here
mtcars$transmission <- ifelse(mtcars$am == 0, "automatic",(ifelse(mtcars$am == 1, "manual","NA")))
...
```{r}
write.csv(clinic, "/home1/leonjche/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/leonjche/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 3.1}
# write code here
head(clinic) #1097 rows, 112 columns
colnames(clinic)
rownames(clinic)
class(clinic$gender) #character
clinic[clinic$Tumor_Sample_Barcode == "TCGA-4H-AAAK","age_at_diagnosis"] #50 years old
clinic[clinic$Tumor_Sample_Barcode == "TCGA-3C-AAAU","age_at_diagnosis"]
clinic[clinic$Tumor_Sample_Barcode == "TCGA-3C-AAAU","last_contact_days_to"]
#55 years old at diagnosis, 3767 days passed = 65 years old at last contact
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$initial_pathologic_dx_year)
```
...
vital_status_cleaned <- clinic$vital_status[vital_status_na_mask]
...
```{r}
under35_over60_mask <- clinic$age_at_diagnosis > 60 | clinic$age_at_diagnosis < 35
```
...
```{r}
cleaned_clinic$age_category <- ifelse(clinic$age_at_diagnosis >= 50, "Old", ifelse(clinic$age_at_diagnosis <= 35, "Young","Middle"))
```
...
cleaned_clinic$survival_time <- ifelse(
  !is.na(cleaned_clinic$death_days_to),
...
Near 0, 15 x 365 = 5475 days is not shown
...
No statistically significant different (p=0.77) across survival rates of all ages groups from 0-4000 days post-diagnosis
